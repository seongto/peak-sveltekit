...
import type { CompanyInfo } from '$lib/interfaces/company-interfaces';


const supabase = createClient(PUBLIC_SUPABASE_URL, PUBLIC_SUPABASE_ANON_KEY);

export async function insertCompany(company: CompanyInfo) {
...
export async function selectCompanyAll(uuid: string) {
    const { data, error } = await supabase
        .from('companies')
